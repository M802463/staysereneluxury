...
villaSelect.addEventListener("click", () => {
  villaOptions.style.display =
    villaOptions.style.display === "block" ? "none" : "block";
});
...
document.querySelectorAll(".guest-controls button").forEach((btn) => {
  btn.addEventListener("click", () => {
    ...
    guestSelect.textContent = `${(adultCount = parseInt(
      document.getElementById("adultCount").textContent
    ))} Adult${adultCount > 1 ? "s" : ""}, ${(childCount = parseInt(
...
document.getElementById("searchBtn").addEventListener("click", () => {
  ...
  const url = `${villaPages[villa]}?dates=${encodeURIComponent(
    dates
